...
<td valign="top" class="barmain" id="menu">
	...
	<?php
		$k = 0;
		foreach($MODULE as $v) {
			if($v['moduleid'] > 4 && !in_array($v['moduleid'], array(51,52))) {
	...
	?>
  <?php
		$menuinc = DT_ROOT.'/module/'.$MODULE[51]['module'].'/admin/menu.inc.php';
		if(is_file($menuinc)) {
			extract($MODULE[51]);
			include $menuinc;
			echo '<dl id="dl_'.$moduleid.'">';
	...
	?>
  <?php
		$menuinc = DT_ROOT.'/module/'.$MODULE[52]['module'].'/admin/menu.inc.php';
		if(is_file($menuinc)) {
			extract($MODULE[52]);
			include $menuinc;
			echo '<dl id="dl_'.$moduleid.'">';
